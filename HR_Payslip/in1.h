...
<body>
      <header>
        ...
        <h1>HR Payslip Management</h1>
        <p>Generate and manage employee payslips efficiently</p>
    </header>
    <div class="container">
        ...
        <div class="form-container" id="form-container">
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="employee-name" placeholder="e.g., Employee Name" maxlength="30">
    ...
    return isValid;
            ...
            async function savePayslip() {
                if (validateForm()) {
                    ...
                    try {
                        const response = await fetch('http://44.203.250.66:3993/api/payslips', {
                            method: 'POST',
            ...
            async function loadPayslips() {
                try {
                    const response = await fetch('http://44.203.250.66:3993/api/payslips/all');
                    if (!response.ok) {
            ...
            async function deletePayslip(id) {
                try {
                    const response = await fetch(`http://44.203.250.66:3993/api/payslips/${id}`, {
                        method: 'DELETE'
                    });
                    if (!response.ok) {
        ...
        function convertWholeNumber(num) {
            ...
            while (remaining > 0) {
                ...
                if (chunk > 0) {
                    ...
                    if (hundreds > 0) {
                        chunkWords += first[hundreds] + 'Hundred ';
                    }

                    if (tensUnits > 0) {
                        if (tensUnits < 20) {
                            chunkWords += first[tensUnits];
                        } else {
                            const tensValue = Math.floor(tensUnits / 10);
                            const onesValue = tensUnits % 10;
                            chunkWords += tens[tensValue];
                            if (onesValue > 0) chunkWords += ' ' + first[onesValue];
                        }
                    }
